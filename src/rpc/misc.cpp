...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"noiraddress\"\n"
            "\nReturn information about the given noir address.\n"
            "\nArguments:\n"
            "1. \"noiraddress\"     (string, required) The noir address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"noiraddress\", (string) The noir address validated\n"
...
UniValue noirsync(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
                "noirsync [status|next|reset]\n"
    ...
    if(strMode == "status") {
        ...
        objStatus.push_back(Pair("AssetID", noirnodeSync.GetAssetID()));
        objStatus.push_back(Pair("AssetName", noirnodeSync.GetAssetName()));
        objStatus.push_back(Pair("Attempt", noirnodeSync.GetAttempt()));
        objStatus.push_back(Pair("IsBlockchainSynced", noirnodeSync.IsBlockchainSynced()));
        objStatus.push_back(Pair("IsNoirnodeListSynced", noirnodeSync.IsNoirnodeListSynced()));
        objStatus.push_back(Pair("IsWinnersListSynced", noirnodeSync.IsWinnersListSynced()));
        objStatus.push_back(Pair("IsSynced", noirnodeSync.IsSynced()));
        objStatus.push_back(Pair("IsFailed", noirnodeSync.IsFailed()));
        return objStatus;
    }

    if(strMode == "next")
    {
        noirnodeSync.SwitchToNextAsset();
        return "sync updated to " + noirnodeSync.GetAssetName();
    }
...
#ifdef ENABLE_WALLET
        // Case 1: noir address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are noir addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) noir address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"noiraddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"noiraddress\"  (string, required) The noir address to use for the signature.\n"
