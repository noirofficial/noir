...
*
*   Why is this needed?
*   - In Noir some blocks are superblocks, which output much higher amounts of coins
...
bool CNoirnodeBlockPayees::IsTransactionValid(const CTransaction &txNew) {
    ...
    LogPrintf("CNoirnodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f NOR\n", strPayeesPossible, (float) nNoirnodePayment / COIN);
    return false;
}
