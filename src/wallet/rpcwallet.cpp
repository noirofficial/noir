...
UniValue spendzerocoinV3(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
                "spendzerocoin <amount>(0.1,0.5,1,10,100) (\"noiraddress\")\n"
                + HelpRequiringPassphrase() +
                "\nArguments:\n"
...
UniValue spendmanyzerocoinV3(const UniValue& params, bool fHelp) {
    ...
    for(size_t i=0; i<inputs.size();i++) {
        ...
        if (!IntegerToDenomination(value * COIN, denomination)) {
            throw runtime_error(
                "spendmanyzerocoin <amount>(0.1,0.5,1,10,100) (\"noiraddress\")\n");
        }
...
UniValue spendmany(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw std::runtime_error(
                ...
                "5. subtractfeefromamount   (string, optional) A json array with addresses.\n"
                "                           The fee will be equally deducted from the amount of each selected address.\n"
                "                           Those recipients will receive less noir than you enter in their corresponding amount field.\n"
                "                           If no addresses are specified here, the sender pays the fee.\n"
                "    [\n"
                "      \"address\"            (string) Subtract fee from this address\n"
                "      ,...\n"
    ...
    for (const auto& strAddr : keys) {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid noir address: " + strAddr);
...
UniValue listspendzerocoinsV3(const UniValue &params, bool fHelp) {
    ...
    for (CWallet::TxItems::const_reverse_iterator it = txOrdered.rbegin(); it != txOrdered.rend(); ++it) {
        ...
        // OP_SIGMAMINT is written. In zerocoin you will see +4 instead,
