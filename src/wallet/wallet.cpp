...
//                continue;
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (nCoinType == ONLY_DENOMINATED) {
                    ...
                    found = !(fZoiNode && pcoin->vout[i].nValue == NOIRNODE_COIN_REQUIRED * COIN);
                } else if (nCoinType == ONLY_NONDENOMINATED_NOT1000IFMN) {
                    if (IsCollateralAmount(pcoin->vout[i].nValue)) continue; // do not use collateral amounts
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if (found && fZoiNode) found = pcoin->vout[i].nValue != NOIRNODE_COIN_REQUIRED * COIN; // do not use Hot MN funds
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector <CTxIn> &vecTxInRet, CAmount &nValueRet,
    ...
    BOOST_FOREACH(const COutput &out, vCoins)
    {
        ...
        if (fZoiNode && out.tx->vout[out.i].nValue == NOIRNODE_COIN_REQUIRED * COIN) continue; //noirnode input
    ...
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
        ...
        for (unsigned int i = 0; i < wtx.vout.size(); i++) {
            ...
            if (fAnonymizable) {
                ...
                if (fZoiNode && wtx.vout[i].nValue == NOIRNODE_COIN_REQUIRED * COIN) continue;
        ...
        CWalletDB(strWalletFile).WriteZerocoinEntry(zerocoinSelected);*/
            ...
            BOOST_FOREACH(const CZerocoinSpendEntry &item, listCoinSpendSerial){
                if (!forceUsed && spend.getCoinSerialNumber() == item.coinSerial) {
                    ...
                    pwalletMain->NotifyZerocoinChanged(pwalletMain, coinToUse.value.GetHex(), "Used(" + std::to_string(pubCoinTx.denomination) + " mint)", CT_UPDATED);
            ...
            pwalletMain->NotifyZerocoinChanged(pwalletMain, coinToUse.value.GetHex(), "Used (" + std::to_string(entry.denomination) + " mint)", CT_UPDATED);
        }
    }
    ...
    if (!CommitZerocoinSpendTransaction(wtxNew, reservekey)) {
        ...
        BOOST_FOREACH(const CZerocoinEntry &pubCoinItem, listPubCoin) {
            if (zcSelectedValue == pubCoinItem.value) {
                ...
                pwalletMain->NotifyZerocoinChanged(pwalletMain, pubCoinItem.value.GetHex(), "New (" + std::to_string(pubCoinTx.denomination) + " mint)", CT_UPDATED);
            }
        }
        CZerocoinSpendEntry entry;
        entry.coinSerial = coinSerial;
